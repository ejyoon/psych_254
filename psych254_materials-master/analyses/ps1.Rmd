...
# just include data that is needed for analysis
d$condition <- d$Input.condition
d$price1 <- d$Input.price1
d$price2 <- d$Input.price2
d$price3 <- d$Input.price3
d$dog_cost <- d$Answer.dog_cost
d$plasma_cost <- d$Answer.plasma_cost
d$sushi_cost <- d$Answer.sushi_cost

# make another data set
d1 <- d %>%
  select(WorkerId, condition, price1, price2, price3, dog_cost, plasma_cost, sushi_cost)
head(d1)
str(d1)
